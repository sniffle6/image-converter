...
Appearance settings include immutable Light, Dark, and Glass themes plus a library of named custom themes. New themes clone the selected appearance, use stable identities when renamed, and can independently retain Solid or Glass material settings. Saved themes are selectable, editable, renameable, and deletable; dirty navigation always asks whether to save, discard, or cancel. Custom color tokens accept opaque `#RRGGBB` or alpha-bearing `#RRGGBBAA` values, and the color picker preserves alpha. A Glass theme uses its window/background color as the native tint and stores its own blur, translucency, and inactive behavior. Versioned theme and JPEG background preferences persist under `%APPDATA%\Convertalot\settings.json`; legacy and transitional single-theme settings migrate automatically, while invalid settings safely fall back to dark defaults.
